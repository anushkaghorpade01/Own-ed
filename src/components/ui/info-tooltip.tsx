    ...
    <Tooltip.Root>
      ...
      <Tooltip.Portal>
        <Tooltip.Content
          ...
          align="center"
          sideOffset={6}
          className="z-50 max-w-[20rem] rounded-lg border border-[var(--border-default)] bg-[var(--surface-card)] px-3 py-2 text-left text-body-sm leading-snug text-[var(--text-secondary)] shadow-md whitespace-pre-line"
        >
          {content}
...
export function MetricLabel({
  ...
  return (
    <p className={cn("text-label inline-flex items-center gap-1.5", className)}>
      <span>{label}</span>
      <InfoTooltip content={tooltip} label={tooltipLabel ?? `About ${label}`} />
    </p>
  );
}
